...
it('keeps process endpoint enforcing groups access checks and context routing', function (): void {
    ...
    expect($template)->toContain("\$context['group_uuid'] = \$group_uuid;");
    expect($template)->toContain("\$context['role'] = \$row_role;");
});
...
it('keeps non-groups bulk upload checks intact in processor', function (): void {
    ...
    expect($template)->toContain("if (\$roster_mode !== 'groups' && \$active_membership_exists(\$membership_uuid, \$email)) {");
