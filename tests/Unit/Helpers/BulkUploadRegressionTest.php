...
it('keeps standalone bulk page strategy-aware with manageable groups selector and auto-redirect', function (): void {
    ...
    expect($template)->toContain('$groups_response = $group_service->getManageableGroups($user_uuid, [');
    ...
    expect($template)->toContain('$group_access = $group_service->canManageGroup($group_uuid, $user_uuid);');
});
...
it('keeps process endpoint enforcing groups access checks and context routing', function (): void {
    ...
    expect($template)->toContain('$roster_mode = (string) $configService->getRosterMode();');
    expect($template)->toContain("if (\$roster_mode === 'groups') {");
    expect($template)->toContain('$group_access = $group_service->canManageGroup($group_uuid, $person_uuid);');
    expect($template)->toContain('$result = $bulk_upload_service->enqueueUpload(');
...
it('keeps non-groups bulk upload checks intact in processor', function (): void {
    ...
    expect($service)->toContain('$this->activeMembershipExists($membership_uuid, $email)');
    expect($service)->toContain('$this->activeMembershipExistsByPerson($membership_uuid, $email)');
});
