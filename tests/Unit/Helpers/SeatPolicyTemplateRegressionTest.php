...
it('keeps member list seat checks wired to effective max assignments resolver', function (): void {
    ...
    expect($unifiedTemplate)->toContain('$max_seats = $membershipService->getEffectiveMaxAssignments($membership_data);');
    expect($legacyTemplate)->toContain('$max_seats = $membershipService->getEffectiveMaxAssignments($membership_data);');
});
...
it('keeps group seat checks wired to effective max assignments resolver', function (): void {
    ...
    expect($groupListTemplate)->toContain('$max_seats = $membershipService->getEffectiveMaxAssignments($membership_data);');
    expect($addGroupMemberProcess)->toContain('$max_seats = $membershipService->getEffectiveMaxAssignments($membership_data);');
});
...
it('keeps organization summary membership tier and seat max wiring through membership service helpers', function (): void {
    ...
    expect($template)->toContain('$membership_name = $membershipService->getMembershipTierName($membership_data);');
    expect($template)->toContain('$max = $membershipService->getEffectiveMaxAssignments($membership_data);');
});
