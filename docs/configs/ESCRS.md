...
- Membership-cycle permissions:
  ...
  - `add_roles = ['membership_owner', 'membership_manager']`
  - `remove_roles = ['membership_owner', 'membership_manager']`
  - `bulk_upload_roles = ['membership_owner', 'membership_manager']`
  - `purchase_seats_roles = ['membership_owner']`
...
- Seats:
  - Per-membership UUID enforcement enabled
  - Capacity alert enabled with custom alert and purchase button text
  - Checkout metadata must include membership UUID
- UI:
  - Unified member view and list enabled
  - Cycle tabs enabled
  - `default_tab = active`
