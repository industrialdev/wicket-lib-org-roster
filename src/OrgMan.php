...
final class OrgMan
{
    ...
    public function addAdditionalSeatsDataToOrderItem($item, $cart_item_key, $values, $order)
    {
        ...
        $context = [
            'source' => 'wicket-orgman',
            'order_id' => is_object($order) && is_callable([$order, 'get_id']) ? (int) $order->get_id() : null,
            'item_id' => is_object($item) && is_callable([$item, 'get_id']) ? (int) $item->get_id() : null,
        ];
        ...
        if (!$additional_seats_product_id) {
            $logger->error('[OrgMan] Additional seats product not found while adding order item meta', $context);
        ...
        if (!$user_meta_data) {
            $logger->warning('[OrgMan] No user meta data found for additional seats order item', array_merge($context, [
                'user_id' => $user_id,
            ]));
        ...
        $logger->info('[OrgMan] Added additional seats data to order item', array_merge($context, [
            'user_id' => $user_id,
            'org_uuid' => $user_meta_data['org_uuid'],
            'membership_id' => $user_meta_data['membership_id'],
        ]));
